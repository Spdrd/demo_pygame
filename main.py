...
def check_queue(root, queue):
    ...
    root.after(10, lambda: check_queue(root, queue))

def moving_window(queue: Queue):
    root = tk.Tk()
    check_queue(root=root, queue=queue)
    root.mainloop()
...
def main():
    queue = Queue()
    thread_bouncing_ball = thr.Thread(target=bouncing_ball, args=(queue,))
    thread_bouncing_ball.start()
    moving_window(queue)
